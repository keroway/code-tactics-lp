...
### CTA クリック計測: UTM パラメータ近似方式

Cloudflare Web Analytics (JavaScript Snippet 版) にはカスタムクリックイベント API が
存在しない。代替として CTA リンクに UTM パラメータを付与し、遷移先サイトの
アクセス解析で LP 経由の流入を区別する。
